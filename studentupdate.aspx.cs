...
public partial class studentupdate : System.Web.UI.Page
{
    ...
    protected void Button2_Click(object sender, EventArgs e)
    {
        string a = TextBox1.Text.ToString();
        conn1();
        string UQ = "update student set FullName='" + TextBox2.Text + "',Email='" + TextBox3.Text + "',Mobile='" + TextBox4.Text + "',UserName='" + TextBox5.Text + "' where StudentId='" + TextBox1.Text.ToString() + "'";
        SqlCommand cmd = new SqlCommand(UQ, cnn);
        cmd.ExecuteNonQuery();
        Response.Write("<h5>" + a.ToString() + " Your Record Sucessfully Updated.." + "</h5>");
        string strSQL = "select * from student";
        DataSet objdataset = new DataSet();
        //sqlConnection objcon = new OleDbConnection(strConnection);
        SqlDataAdapter objadapter = new SqlDataAdapter(strSQL, cnn);
